...
if (void 0 != performance.memory) {
    if (
        (hasMemApi = true),
        (startMem = performance.memory),
        161e5 == performance.memory.totalJSHeapSize ||
        127e5 == performance.memory.usedJSHeapSize ||
        1e7 == performance.memory.usedJSHeapSize ||
        219e4 == performance.memory.jsHeapSizeLimit
    ) {
        for (hasKnownMem = true; calcSolution(performance.memory.usedJSHeapSize);) {
            if (0 > performance.now()) {
                hasKnownMem = false;
            }
        }
    } else {
...
if (hasMemApi) {
    ...
    if (
        startMem.usedJSHeapSize == e.usedJSHeapSize &&
        startMem.jsHeapSizeLimit == e.jsHeapSizeLimit &&
        startMem.totalJSHeapSize == e.totalJSHeapSize
    ) {
        for (useMem = true; calcSolution(performance.memory.usedJSHeapSize);) {
            if (0 > performance.now()) {
                hasKnownMem = false;
            }
        }
...
let pluginString = Object.getOwnPropertyDescriptor(
    Object.getPrototypeOf(navigator),
    "plugins"
).get.toString();

if (
    "function get plugins() { [native code] }" != pluginString &&
    "function plugins() {\n        [native code]\n    }" != pluginString &&
    "function plugins() {\n    [native code]\n}" != pluginString
) {
    plugCh = true;
...
let mimeString = Object.getOwnPropertyDescriptor(
    Object.getPrototypeOf(navigator),
    "plugins"
).get.toString();

if (
    "function get plugins() { [native code] }" != mimeString &&
    "function plugins() {\n        [native code]\n    }" != mimeString &&
    "function plugins() {\n    [native code]\n}" != mimeString
) {
    mimeCh = true;
  ...
  function bscctty(){
    if(!tmfgdl){
      ...
      const secret = "wdhdwsakjdhalkdjhklhk43khkj24h44jk3hjk4h324l432";
      ...
      const secret = "ksdhakdhsjdhkjadhhjh2lh321kj3hkl2j3h21ljk3h123jkl23h2312";
      const separator = "?token";
      const time = new Date/1000 | 0;
  ...
  function sap(){
    ...
    obj.onload = function() { obj.remove(), sapr = true, jbcpl = true, dpass = true, document.cookie = "sap_s=1; SameSite=None; path=/; Secure" }
    obj.onerror = function() { obj.remove(), document.cookie = "wpdd=gtfr-zggb-uihbn; SameSite=None; path=/; Secure", sapr = true, dpass = true, document.cookie = "sap_e=1; SameSite=None; path=/; Secure" }
  ...
  function eop(){
    ...
    obj.onload = function() { obj.remove(), sapr = true, dpass = true, document.cookie = "eop_s=1; SameSite=None; path=/; Secure" }
    obj.onerror = function() { obj.remove(), sapr = false, dpass = true, document.cookie = "eop_e=1; SameSite=None; path=/; Secure" }
    document.body.appendChild(obj);
  ...
  */
  ...
  function chcim(){
    ...
    chcimage.src = "http://Cryptwarerunsall.jg";
    chcimage.setAttribute("id", "Cryptwaremitigation");
  ...
  function ifsfb(){
    const iframe = document.createElement("iframe");
    iframe.srcdoc = `<!DOCTYPE html><p>Cryptware runs all</p>`;
...
if (tmfgdl == true) {
    const valueToEncrypt = "asdiasjdasdjiodasd"; // Value to be encrypted
    const encoder = new TextEncoder();
    const data = encoder.encode(valueToEncrypt);
  
    crypto.subtle.digest("SHA-256", data).then(function (hash) {
      const hashArray = Array.from(new Uint8Array(hash));
      const hashedValue = hashArray.map(byte => byte.toString(16).padStart(2, '0')).join('');
  
      document.cookie = "s2_s_cc=" + hashedValue + "; SameSite=Lax; path=/; Secure";
